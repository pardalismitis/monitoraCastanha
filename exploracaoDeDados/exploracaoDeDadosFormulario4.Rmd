...
#leitura
custo = read.csv(here("data", "formulario4Custo.csv"), sep = ";")

custo = limpezaGeral(custo)
...
for(i in 1:ncol(custo)) {
  if(is.character(custo[, i])){
  custo[ , i] <- chartr(AccChars, RegChars, custo[ , i])
  custo[ , i] <- tolower(custo[ , i])
}
}

custo$nomeCastanhal <- gsub("marizan_","marizan",
                                gsub("acapulco","acapu",
                                gsub("ouro_preto","Rio Ouro Preto",
                                gsub("cazumba","Cazumbá-Iracema",
                                     custo$nomeCastanhal))))

custo$Tempo_total <- as.integer(custo$Tempo_total)
custo$Tempo_total <- (custo$Temp_chegar_castanhal_dias*1440)+(custo$Temp_chegar_castanhal_hor*60)+custo$Temp_chegar_castanhal_min


#### Juntando castanhais por representate - Formulário 4
grupo_representante=group_by(custo, nomeCastanhal, UC, ANO)



Custo2 = summarise(grupo_representante, 
                       producao_total = sum(as.integer(producao_total)),
                       Custo_alimentacao = sum(as.integer(Custos_Alimentacao)),
                        Gasto_combus_frete = sum(as.integer(Gasto_combus_frete)), 
                       Gasto_manutencao = sum(as.integer(Gasto_manutencao)), 
                       Gasto_diaria = sum(as.integer(Gasto_diaria)),
                       Insumos_gastos = sum(as.integer(Insumos_gastos)),
                       EPI_gastos = sum(as.integer(EPI_gastos)),
                       Gastos_total = sum(as.integer(Gastos_total)),
                            Custo = sum(as.integer(Custo)),
                        Total_kg_produzido = sum(as.integer(Total_kg_produzido)),
                        Tempo_total = sum(as.integer(Tempo_total)),
                    Dias_trabalhando = sum(as.integer(Dias_trabalhando)))

cazumba <- subset(Custo2, Custo2$UC == "cazumba")

ggplot(
  Custo2, aes(x = ANO, y =producao_total)
)+
